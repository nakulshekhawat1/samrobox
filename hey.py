...
class VoiceRobot:
    def __init__(self, root):
        ...
        self.scale = min(self.screen_width, self.screen_height) / 400
    ...
    def draw_robot(self, state="normal", mouth_open=0):
        ...
        head_y = center_y - head_size // 2 - 50 * s
        
        # Head background
        self.canvas.create_rectangle(head_x, head_y, head_x + head_size, head_y + head_size,
                                    fill='#1a1a2e', outline='', tags="head")
        
        # Eyes - Simple dots
        eye_radius = 18 * s
        eye_y = center_y - 20 * s
        left_eye_x = center_x - 80 * s
        right_eye_x = center_x + 80 * s
        
        # Left eye
        self.canvas.create_oval(left_eye_x - eye_radius, eye_y - eye_radius,
                               left_eye_x + eye_radius, eye_y + eye_radius,
                               fill=eye_color, outline='', tags="left_eye")
        
        # Right eye
        self.canvas.create_oval(right_eye_x - eye_radius, eye_y - eye_radius,
                               right_eye_x + eye_radius, eye_y + eye_radius,
                               fill=eye_color, outline='', tags="right_eye")
        
        # Pupils
        pupil_radius = 6 * s
        self.canvas.create_oval(left_eye_x - pupil_radius, eye_y - pupil_radius,
                               left_eye_x + pupil_radius, eye_y + pupil_radius,
                               fill='black', outline='', tags="left_pupil")
        self.canvas.create_oval(right_eye_x - pupil_radius, eye_y - pupil_radius,
                               right_eye_x + pupil_radius, eye_y + pupil_radius,
                               fill='black', outline='', tags="right_pupil")
        
        # Listening pulse effect
        if state == "listening":
            pulse_radius = eye_radius + 15 * s
            self.canvas.create_oval(left_eye_x - pulse_radius, eye_y - pulse_radius,
                                   left_eye_x + pulse_radius, eye_y + pulse_radius,
                                   outline=eye_color, width=2*s, tags="left_pulse")
            self.canvas.create_oval(right_eye_x - pulse_radius, eye_y - pulse_radius,
                                   right_eye_x + pulse_radius, eye_y + pulse_radius,
                                   outline=eye_color, width=2*s, tags="right_pulse")
        ...
        if state == "speaking":
            mouth_height = 5 + (mouth_open / 100) * 15
            self.canvas.create_line(center_x - mouth_width//2, mouth_y - mouth_height,
                                   center_x + mouth_width//2, mouth_y - mouth_height,
                                   fill=mouth_color, width=int(4*s), tags="mouth")
        elif state == "listening":
            self.canvas.create_arc(center_x - mouth_width//2, mouth_y - 10*s,
                                  center_x + mouth_width//2, mouth_y + 10*s,
                                  start=0, extent=180, style='arc',
                                  outline=mouth_color, width=int(4*s))
            for i in range(3):
                x_offset = (20 + i * 12) * s
                self.canvas.create_arc(center_x - mouth_width//2 - x_offset, mouth_y - 15*s,
                                       center_x - mouth_width//2 - x_offset + 15*s, mouth_y + 15*s,
        ...
        if state == "listening":
            dot_color = '#9b59b6'
            self.canvas.create_oval(self.screen_width - 50, self.screen_height - 30,
                                   self.screen_width - 42, self.screen_height - 22,
                                   fill=dot_color, outline=dot_color)
            self.canvas.create_oval(self.screen_width - 42, self.screen_height - 35,
                                   self.screen_width - 34, self.screen_height - 27,
                                   fill=dot_color, outline=dot_color)
            self.canvas.create_oval(self.screen_width - 46, self.screen_height - 40,
                                   self.screen_width - 38, self.screen_height - 32,
                                   fill=dot_color, outline=dot_color)
        elif state == "speaking":
            dot_color = '#3498db'
            self.canvas.create_oval(self.screen_width - 50, self.screen_height - 30,
            ...
            self.canvas.create_oval(self.screen_width - 46, self.screen_height - 40,
                                   self.screen_width - 38, self.screen_height - 32,
                                   fill=dot_color, outline=dot_color)
            self.canvas.create_oval(self.screen_width - 54, self.screen_height - 35,
                                   self.screen_width - 46, self.screen_height - 27,
                                   fill=dot_color, outline=dot_color)
